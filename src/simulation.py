...
model1.load_state_dict(
    torch.load("models/train_mask_rCNN_100k.pth", map_location=DEVICE)
)
model1.eval()
...
model2.load_state_dict(
    torch.load("models/Lane_Model_ENet_okey.pth", map_location=DEVICE)
)
model2.eval()
...
interval = int(fps)
print(
    f"The video will be processed every {interval} frames to have a 1-second refresh."
)
...
if GraphMode == 0 or GraphMode == 3 or GraphMode == 5:
    output_video = cv2.VideoWriter(
        ...
        1.0,
        (2 * target_size[1], 2 * target_size[0]),
    )
if GraphMode == 1 or GraphMode == 2:
    output_video = cv2.VideoWriter(
        ...
        cv2.VideoWriter_fourcc(*"mp4v"),
        1.0,
        (2 * target_size[1] * 3, 2 * target_size[0]),
    )
if GraphMode == 4:
    output_video = cv2.VideoWriter(
        output_video_file,
        cv2.VideoWriter_fourcc(*"mp4v"),
        1.0,
        (2 * target_size[1] * 2, 2 * target_size[0]),
    )
...
if GraphMode == 0:
    fig, axes = plt.subplots(
        1, 1, figsize=(2 * target_size[1] * 0.01, 2 * target_size[0] * 0.01)
    )
    # fig.subplots_adjust(left=0.05, right=0.95, top=0.9, bottom=0.1)
    im_original = axes.imshow(
...
if GraphMode == 1:
    fig, axes = plt.subplots(
        1, 3, figsize=(2 * target_size[1] * 0.03, 2 * target_size[0] * 0.01)
    )
    # fig.subplots_adjust(left=0.05, right=0.95, top=0.9, bottom=0.1)
    ...
    axes[0].axis("off")
    im_predicted_mask_rCNN = axes[1].imshow(
        np.ones((target_size[0], target_size[1]), dtype=np.uint8),
        cmap="gray",
        vmin=0,
        vmax=1,
    )
    ...
    axes[1].axis("off")
    im_predicted_LaneNet = axes[2].imshow(
        np.ones((target_size[0], target_size[1]), dtype=np.uint8),
        cmap="gray",
        vmin=0,
        vmax=1,
...
if GraphMode == 4:
    cmap_colour = LinearSegmentedColormap.from_list(
        "colour", [(0, "red"), (1, "white")]
    )
    fig, axes = plt.subplots(
        1, 2, figsize=(2 * target_size[1] * 0.02, 2 * target_size[0] * 0.01)
    )
    # fig.subplots_adjust(left=0.05, right=0.95, top=0.9, bottom=0.1)
    im_original1 = axes[0].imshow(
        np.zeros((target_size[0], target_size[1], 3), dtype=np.uint8)
    )
    # im_mask1 = axes[0].imshow(np.zeros((target_size[0], target_size[1]), dtype=np.uint8), cmap='gray', vmin=0, vmax=1, alpha=0.1)
    im_mask1 = axes[0].imshow(
    ...
    axes[0].axis("off")
    im_original2 = axes[1].imshow(
        np.zeros((target_size[0], target_size[1], 3), dtype=np.uint8)
    )
    im_mask2 = axes[1].imshow(
        np.zeros((target_size[0], target_size[1]), dtype=np.uint8),
        cmap=cmap_colour,
        vmin=0,
        vmax=1,
    ...
    if frame_count % interval == 0:
        ...
        with torch.no_grad():
            ...
            output1 = F.interpolate(
                output1, size=target_size, mode="bilinear", align_corners=False
            )
        ...
        with torch.no_grad():
            ...
            output2 = (
                torch.squeeze(output2["instance_seg_logits"], dim=0)
                .transpose(1, 2)
                .transpose(0, 2)
            )
            weights2 = torch.tensor([0.2989, 0.5870, 0.1140], device=DEVICE).view(
                1, 1, 3
            )
        ...
        if GraphMode == 0 or GraphMode == 2 or GraphMode == 5:
            ...
            for c in range(3):
                image[
                    c,
                    y_offset : y_offset + wheel_image_rotated.size(1),
                    x_offset : x_offset + wheel_image_rotated.size(2),
                ] = (
                    image[
                        c,
                        y_offset : y_offset + wheel_image_rotated.size(1),
        ...
        if GraphMode == 3:
            ...
            angle = angle + angle1 * 2
            rotate_transform = transforms.functional.rotate
            wheel_image_rotated_img = rotate_transform(
                wheel_image_resized, angle=angle_img
            )
            wheel_image_rotated = rotate_transform(wheel_image_resized, angle=angle)
            ...
            for c in range(3):
                image[
                    c,
                    y_offset_img : y_offset_img + wheel_image_rotated_img.size(1),
                    x_offset_img : x_offset_img + wheel_image_rotated_img.size(2),
                ] = (
                    image[
                        c,
                        y_offset_img : y_offset_img + wheel_image_rotated_img.size(1),
