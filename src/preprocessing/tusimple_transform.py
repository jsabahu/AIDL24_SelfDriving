...
def init_args():
    ...
    parser.add_argument(
        "--src_dir", type=str, help="The origin path of unzipped tusimple dataset"
    )
    parser.add_argument("--val", type=bool, help="Tag for validation set", default=True)
    parser.add_argument(
        "--test", type=bool, help="Tag for validation set", default=False
    )
    ...
    with open(json_file_path, "r") as file:
        for line_index, line in enumerate(file):
            ...
            for lane_index, lane in enumerate(lanes):
                ...
                cv2.polylines(
                    dst_binary_image, lane_pts, isClosed=False, color=255, thickness=5
                )
                cv2.polylines(
                    dst_instance_image,
                    lane_pts,
